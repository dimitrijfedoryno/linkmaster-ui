...
export const startDownload = async (downloadId, tracks, format, url, playlistTitle, io, ownerId) => {
    let destinationPath = format === 'audio' ? DOWNLOAD_PATH_AUDIO : DOWNLOAD_PATH_VIDEO;

    // Platform specific subfolders
    if (tracks.length > 0) {
        const type = tracks[0].type;
        if (type === 'twitch') {
            destinationPath = path.join(destinationPath, 'Twitch');
        } else if (type === 'kick') {
            destinationPath = path.join(destinationPath, 'Kick');
        } else if (type === 'soundcloud') {
            destinationPath = path.join(destinationPath, 'SoundCloud');
        }
    }

    // For playlists, create a dedicated folder inside the platform folder
    if (tracks.length > 1) {
        const safeFolderName = (playlistTitle || 'Download').replace(/[<>:"/\\|?*]/g, '').trim();
        destinationPath = path.join(destinationPath, safeFolderName);
    }
